    ...
    <AnimatePresence>
      {file && (
        ...
        className="fixed -bottom-[120%] aspect-square w-[120vw] -left-[10vw]">
          <div className="loading-indicator blob rounded-full w-full h-full" style={{
      ...
      <AnimatePresence>
        <Dialog open={!parseResults}>
          <DialogContent forceMount className="focus:outline-none">
            <motion.div initial="initial" animate={videoLoaded ? "animate" : "initial"} transition={{
              staggerChildren: 0.08,
              delayChildren: 0.2,
            }} className="flex flex-col gap-8">
              <MotionHeader>
                <AnimatePresence>
                <MotionTitle variants={{
                  initial: {
                    y: 20,
                    opacity: 0,
                    filter: "blur(4px)"
                ...
                }} key={loading ? "Uploading" : "Upload"}>{loading ? "Uploading" : "Upload"} Audit</MotionTitle>
                </AnimatePresence>
              </MotionHeader>
              <AnimatePresence>
              {loading ? (
                <div className="w-[29rem] relative h-96">
                  ...
                  <LoadingBlob key="blob-1" x={[-230, -100, -230]} y={[-120, -200, -120]} xDuration={14} yDuration={15} />
                  <LoadingBlob key="blob-2" x={[-80, -250, -80]} y={[-200, -160, -200]} xDuration={14} yDuration={8} />
                  <LoadingBlob key="blob-3" x={[-90, -220, -90]} y={[-200, -120, -200]} xDuration={6} yDuration={12} />
                </div>
              ) : (
                <>
              ...
              <AnimatePresence>
                ...
                />
                  ...
                  >
                    <div className={`w-full h-full border-[6px] border-dashed rounded-lg flex justify-center items-center transition-all ${isDragActive ? "border-violet-500" : "border-zinc-800"}`} {...getRootProps()}>
                      <input {...getInputProps()} />
                      <AnimatePresence>
                        {file ? (
                          <>
                            ...
                            className="absolute -bottom-[160%] aspect-square w-[160%] -z-10">
                              <div className="bg-gradient-to-br from-violet-600 via-purple-700 to-pink-800 blob w-full h-full" style={{
                          ...
                          <Upload className={`transition-all ${isDragActive ? "text-violet-500 size-10" : "text-zinc-400 size-6"}`} />
                        )}
                      </AnimatePresence>
                    </div>
                  </motion.div>
                  {!isDragActive && !file && (
                    ...
                    }} key={`heading-${step}`} className="font-medium text-center whitespace-nowrap inline-block text-sm text-zinc-200 absolute bottom-4 bg-background/80 backdrop-blur-md border-border border px-2 py-1 rounded-md left-1/2">
                      {step == 0 ? (
...
function LoadingBlob({ x, y, xDuration, yDuration }: {
  x: number[],
  y: number[],
  xDuration: number,
  yDuration: number,
